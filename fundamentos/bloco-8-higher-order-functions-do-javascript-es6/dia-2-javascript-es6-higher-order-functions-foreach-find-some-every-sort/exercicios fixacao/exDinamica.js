...
const verifyAges = (arr, minimumAge) => {
  const result = arr.every((age) => age >= minimumAge);

  return result ? 'Beba Trybeer, suave como um log de erro vazio' : 'ChocoTrybe, melhor que console.log';
}

console.log(verifyAges(arrayIdades,18));
console.log(verifyAges(arrayIdades2,18));
...
const verifyAprovall = () => {
  const aprovall = estudantes.every((item) => item.status >= 80);

  return aprovall ? 'Pode abrir o suco de laranja que temos 100% de aprovação!' : 'Bora pra mentoria de projetos!';
}

console.log(verifyAprovall());
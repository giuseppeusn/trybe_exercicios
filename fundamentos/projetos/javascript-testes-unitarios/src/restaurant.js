...
const sumValues = (order, obj) => {
  let sum = 0;

  for (let key in obj) {
    if (Object.keys(obj[key]).indexOf(order) > -1) {
      sum += obj[key][order];
    }
  }
...
const createMenu = (obj) => { 
  ...
  const menu = {
    fetchMenu: () => obj,
    consumption: [],
    order: consum = (string) => menu.consumption.push(string),
    pay: value = () => {
      let sum = 0;

      for (let x = 0; x < menu.consumption.length; x += 1) {
        let order = menu.consumption[x];
        sum += sumValues(order, obj);
      }

      return sum + (10 / 100) * sum;
    },
  };
